// DO NOT EDIT THIS FILE
// GENERATED ON 2024-05-04 00:04:23

package love.audio;

import haxe.extern.Rest;
import lua.Table;
import lua.UserData;

@:native("love.audio")
extern class Audio {

	public static function getActiveEffects(): Table<Dynamic,Dynamic>;

	public static function getActiveSourceCount(): Float;

	public static function getDistanceModel(): Dynamic;

	public static function getDopplerScale(): Float;

	public static function getEffect(name:String): Table<Dynamic,Dynamic>;

	public static function getMaxSceneEffects(): Float;

	public static function getMaxSourceEffects(): Float;

	public static function getOrientation(): OrientationResult;

	public static function getPosition(): PositionResult;

	public static function getRecordingDevices(): Table<Dynamic,Dynamic>;

	public static function getSourceCount(): Float;

	public static function getVelocity(): VelocityResult;

	public static function getVolume(): Float;

	public static function isEffectsSupported(): Bool;

	public static function newQueueableSource(samplerate:Float, bitdepth:Float, channels:Float, buffercount:Float = Dynamic): Dynamic;

	@:overload
	public static function newSource(filename:String, type:Dynamic): Dynamic;

	@:overload
	public static function newSource(file:Dynamic, type:Dynamic): Dynamic;

	@:overload
	public static function newSource(decoder:Dynamic, type:Dynamic): Dynamic;

	@:overload
	public static function newSource(data:Dynamic, type:Dynamic): Dynamic;

	@:overload
	public static function newSource(data:Dynamic): Dynamic;

	@:overload
	public static function pause(): Table<Dynamic,Dynamic>;

	@:overload
	public static function pause(source:Dynamic, ...:Dynamic): Void;

	@:overload
	public static function pause(sources:Table<Dynamic,Dynamic>): Void;

	@:overload
	public static function play(source:Dynamic): Void;

	@:overload
	public static function play(sources:Table<Dynamic,Dynamic>): Void;

	@:overload
	public static function play(source1:Dynamic, source2:Dynamic, ...:Dynamic): Void;

	public static function setDistanceModel(model:Dynamic): Void;

	public static function setDopplerScale(scale:Float): Void;

	@:overload
	public static function setEffect(name:String, settings:Table<Dynamic,Dynamic>): Bool;

	@:overload
	public static function setEffect(name:String, enabled:Bool = Dynamic): Bool;

	public static function setMixWithSystem(mix:Bool): Bool;

	public static function setOrientation(fx, fy, fz:Float, ux, uy, uz:Float): Void;

	public static function setPosition(x:Float, y:Float, z:Float): Void;

	public static function setVelocity(x:Float, y:Float, z:Float): Void;

	public static function setVolume(volume:Float): Void;

	@:overload
	public static function stop(): Void;

	@:overload
	public static function stop(source:Dynamic): Void;

	@:overload
	public static function stop(source1:Dynamic, source2:Dynamic, ...:Dynamic): Void;

	@:overload
	public static function stop(sources:Table<Dynamic,Dynamic>): Void;

}

extern public class OrientationResult {
	public var fx, fy, fz:Float;
	public var ux, uy, uz:Float;
}

extern public class PositionResult {
	public var x:Float;
	public var y:Float;
	public var z:Float;
}

extern public class VelocityResult {
	public var x:Float;
	public var y:Float;
	public var z:Float;
}

extern class RecordingDevice extends Object {

	public static function getBitDepth(): Float;

	public static function getChannelCount(): Float;

	public static function getData(): Dynamic;

	public static function getName(): String;

	public static function getSampleCount(): Float;

	public static function getSampleRate(): Float;

	public static function isRecording(): Bool;

	public static function start(samplecount:Float, samplerate:Float = Dynamic, bitdepth:Float = Dynamic, channels:Float = Dynamic): Bool;

	public static function stop(): Dynamic;

}

extern class Source extends Object {

	public static function clone(): Dynamic;

	public static function getActiveEffects(): Table<Dynamic,Dynamic>;

	public static function getAirAbsorption(): Float;

	public static function getAttenuationDistances(): AttenuationDistancesResult;

	public static function getChannelCount(): Float;

	public static function getCone(): ConeResult;

	public static function getDirection(): DirectionResult;

	public static function getDuration(unit:Dynamic = Dynamic): Float;

	public static function getEffect(name:String, filtersettings:Table<Dynamic,Dynamic> = Dynamic): Table<Dynamic,Dynamic>;

	public static function getFilter(): Table<Dynamic,Dynamic>;

	public static function getFreeBufferCount(): Float;

	public static function getPitch(): Float;

	public static function getPosition(): PositionResult;

	public static function getRolloff(): Float;

	public static function getType(): Dynamic;

	public static function getVelocity(): VelocityResult;

	public static function getVolume(): Float;

	public static function getVolumeLimits(): VolumeLimitsResult;

	public static function isLooping(): Bool;

	public static function isPlaying(): Bool;

	public static function isRelative(): Bool;

	public static function pause(): Void;

	public static function play(): Bool;

	public static function queue(sounddata:Dynamic): Bool;

	public static function seek(offset:Float, unit:Dynamic = Dynamic): Void;

	public static function setAirAbsorption(amount:Float): Void;

	public static function setAttenuationDistances(ref:Float, max:Float): Void;

	public static function setCone(innerAngle:Float, outerAngle:Float, outerVolume:Float = Dynamic): Void;

	public static function setDirection(x:Float, y:Float, z:Float): Void;

	@:overload
	public static function setEffect(name:String, enable:Bool = Dynamic): Bool;

	@:overload
	public static function setEffect(name:String, filtersettings:Table<Dynamic,Dynamic>): Bool;

	@:overload
	public static function setFilter(settings:Table<Dynamic,Dynamic>): Bool;

	@:overload
	public static function setFilter(): Void;

	public static function setLooping(loop:Bool): Void;

	public static function setPitch(pitch:Float): Void;

	public static function setPosition(x:Float, y:Float, z:Float): Void;

	public static function setRelative(enable:Bool = Dynamic): Void;

	public static function setRolloff(rolloff:Float): Void;

	public static function setVelocity(x:Float, y:Float, z:Float): Void;

	public static function setVolume(volume:Float): Void;

	public static function setVolumeLimits(min:Float, max:Float): Void;

	public static function stop(): Void;

	public static function tell(unit:Dynamic = Dynamic): Float;

}

enum abstract DistanceModel {
	var None = "none";
	var Inverse = "inverse";
	var Inverseclamped = "inverseclamped";
	var Linear = "linear";
	var Linearclamped = "linearclamped";
	var Exponent = "exponent";
	var Exponentclamped = "exponentclamped";
}

enum abstract EffectType {
	var Chorus = "chorus";
	var Compressor = "compressor";
	var Distortion = "distortion";
	var Echo = "echo";
	var Equalizer = "equalizer";
	var Flanger = "flanger";
	var Reverb = "reverb";
	var Ringmodulator = "ringmodulator";
}

enum abstract EffectWaveform {
	var Sawtooth = "sawtooth";
	var Sine = "sine";
	var Square = "square";
	var Triangle = "triangle";
}

enum abstract FilterType {
	var Lowpass = "lowpass";
	var Highpass = "highpass";
	var Bandpass = "bandpass";
}

enum abstract SourceType {
	var Static = "static";
	var Stream = "stream";
	var Queue = "queue";
}

enum abstract TimeUnit {
	var Seconds = "seconds";
	var Samples = "samples";
}

