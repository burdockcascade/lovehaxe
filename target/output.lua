...
local EReg = _hx_e()
local Generate = _hx_e()
local Generator = _hx_e()
local Lambda = _hx_e()
local Math = _hx_e()
local Reflect = _hx_e()
local String = _hx_e()
...
Generate.new = {}
Generate.__name__ = true
Generate.main = function() 
  local http = __sys_Http.new(Generate.LOVE_API_URL);
  http.onData = function(self,data) 
    local _hx_status, _hx_result = pcall(function() 
    
        Generator.new(__haxe_Json.parse(data));
      return _hx_pcall_default
    end)
    if not _hx_status and _hx_result == "_hx_pcall_break" then
    elseif not _hx_status then 
      local _g = _hx_result;
      local error = __haxe_Exception.caught(_g):unwrap();
      __haxe_Log.trace(Std.string("Error parsing JSON data: ") .. Std.string(Std.string(error)), _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/Generate.hx",lineNumber=19,className="Generate",methodName="main"}));
    elseif _hx_result ~= _hx_pcall_default then
      return _hx_result
    end;
   end;
  http.onError = function(self,error) 
    __haxe_Log.trace(Std.string("Error downloading JSON: ") .. Std.string(error), _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/Generate.hx",lineNumber=24,className="Generate",methodName="main"}));
   end;
  http:request();
end

Generator.new = function(apiData) 
  local self = _hx_new(Generator.prototype)
  Generator.super(self,apiData)
  return self
end
Generator.super = function(self,apiData) 
  self.structTracker = __haxe_ds_StringMap.new();
  self.love2dVersion = apiData.version;
  __haxe_Log.trace(Std.string("Generating bindings for LOVE version ") .. Std.string(self.love2dVersion), _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/Generate.hx",lineNumber=39,className="Generator",methodName="new"}));
  local _g = 0;
  local _g1 = _hx_wrap_if_string_field(apiData.modules,'length');
  while (_g < _g1) do _hx_do_first_1 = false;
    
    _g = _g + 1;
    local i = _g - 1;
    self:writeModule(apiData.modules[i]);
  end;
end
Generator.__name__ = true
Generator.mapType = function(type) 
  local type1 = type;
  ...
  elseif (type1) == "table" then 
    do return "Table<Dynamic,Dynamic>" end;else
  do return Generator.capitalizeFirstLetter(type) end; end;
end
Generator.capitalizeFirstLetter = function(str) 
  do return Std.string(__lua_lib_luautf8_Utf8.upper(__lua_lib_luautf8_Utf8.sub(str, 1, 1))) .. Std.string(String.prototype.substr(str, 1)) end;
end
Generator.prototype = _hx_e();
Generator.prototype.writeModule = function(self,module) 
  local mod_name = module.name;
  local mod_functions = module.functions;
  local mod_types = module.types;
  local mod_constants = module.enums;
  self.structTracker = __haxe_ds_StringMap.new();
  __haxe_Log.trace(Std.string("Generating bindings for module ") .. Std.string(mod_name), _hx_o({__fields__={fileName=true,lineNumber=true,className=true,methodName=true},fileName="src/Generate.hx",lineNumber=55,className="Generator",methodName="writeModule"}));
  local class_name = Generator.capitalizeFirstLetter(mod_name);
  local fout = __sys_io_File.write(Std.string(Std.string("lib/love/") .. Std.string(class_name)) .. Std.string(".hx"));
  ...
  local gen_date = d;
  fout:writeString(Std.string(Std.string(Std.string(Std.string("/*\n * This file was generated by the LOVE2D Haxe bindings generator for Love2D v") .. Std.string(self.love2dVersion)) .. Std.string(".\n * Do not modify this file directly.\n * ")) .. Std.string(Std.string(gen_date))) .. Std.string("\n */\n\n"));
  fout:writeString("package love;\n\n");
  fout:writeString(Std.string(Std.string("@:native(\"love.") .. Std.string(mod_name)) .. Std.string("\")\n"));
  self:writeClass(fout, class_name, mod_functions);
  if (mod_types ~= nil) then 
    self:writeTypes(fout, mod_types);
  end;
  if (mod_constants ~= nil) then 
    self:writeEnums(fout, mod_constants);
  end;
  self:writeFunctionReturnStructs(fout, module);
end
Generator.prototype.writeTypes = function(self,fout,types) 
  local _g = 0;
  local _g1 = types.length;
  while (_g < _g1) do _hx_do_first_1 = false;
    
    _g = _g + 1;
    local i = _g - 1;
    local hxType = types[i];
    local className = Generator.capitalizeFirstLetter(hxType.name);
    fout:writeString(Std.string(Std.string("extern class ") .. Std.string(className)) .. Std.string(" {\n\n"));
    if (hxType.functions ~= nil) then 
      local _g = 0;
      local _g1 = hxType.functions.length;
      while (_g < _g1) do _hx_do_first_2 = false;
        
        _g = _g + 1;
        local i = _g - 1;
        self:writeFunction(fout, hxType.functions[i]);
      end;
    end;
    fout:writeString("}\n\n");
  end;
end
Generator.prototype.writeClass = function(self,fout,class_name,functions) 
  fout:writeString(Std.string(Std.string("extern class ") .. Std.string(class_name)) .. Std.string(" {\n\n"));
  local _g = 0;
  local _g1 = functions.length;
  while (_g < _g1) do _hx_do_first_1 = false;
    
    _g = _g + 1;
    local i = _g - 1;
    self:writeFunction(fout, functions[i]);
  end;
  fout:writeString("}\n\n");
end
Generator.prototype.writeFunctionReturnStructs = function(self,fout,module) 
  local _g = 0;
  local _g1 = _hx_wrap_if_string_field(module.types,'length');
  while (_g < _g1) do _hx_do_first_1 = false;
    
    _g = _g + 1;
    local i = _g - 1;
    local hxType = module.types[i];
    if (hxType.functions ~= nil) then 
      local _g = 0;
      local _g1 = hxType.functions.length;
      while (_g < _g1) do _hx_do_first_2 = false;
        ...
        _g = _g + 1;
        local i = _g - 1;
        local func = hxType.functions[i];
        self:writeFunctionReturnStruct(fout, func);
      end;
    end;
  end;
  local _g = 0;
  local _g1 = _hx_wrap_if_string_field(module.functions,'length');
  while (_g < _g1) do _hx_do_first_1 = false;
    
    _g = _g + 1;
    local i = _g - 1;
    local func = module.functions[i];
    self:writeFunctionReturnStruct(fout, func);
  end;
end
Generator.prototype.writeFunctionReturnStruct = function(self,fout,func) 
  local variants = func.variants;
  local funcName = func.name;
  local _g = 0;
  local _g1 = variants.length;
  while (_g < _g1) do _hx_do_first_1 = false;
    
    _g = _g + 1;
    local i = _g - 1;
    local variant = variants[i];
    if ((variant.returns ~= nil) and (variant.returns.length > 1)) then 
      local className = self:getFunctionStructName(funcName);
      local ret = self.structTracker.h[className];
      if ((function() 
        local _hx_1
      ...
      local _this = self.structTracker;
      _this.h[className] = true;
      fout:writeString(Std.string(Std.string("extern class ") .. Std.string(className)) .. Std.string(" {\n\n"));
      local _g = 0;
      local _g1 = variant.returns.length;
      while (_g < _g1) do _hx_do_first_2 = false;
        
        _g = _g + 1;
        local i = _g - 1;
        local variantReturn = variant.returns[i];
        local returnName = variantReturn.name;
        local returnType = Generator.mapType(variantReturn.type);
        fout:writeString(Std.string(Std.string(Std.string(Std.string("\tpublic var ") .. Std.string(returnName)) .. Std.string(":")) .. Std.string(returnType)) .. Std.string(";\n"));
      end;
      fout:writeString("\n}\n\n");
    end;
  end;
end
Generator.prototype.getFunctionStructName = function(self,funcName) 
  if (String.prototype.indexOf(funcName, "get") == 0) then 
    funcName = String.prototype.substr(funcName, 3);
  else
    funcName = Std.string(funcName) .. Std.string("Result");
  end;
  funcName = Generator.capitalizeFirstLetter(funcName);
  do return funcName end
end
Generator.prototype.writeFunction = function(self,fout,func) 
  local identation = "\t";
  local variants = func.variants;
  ...
  while (_g < _g1) do _hx_do_first_1 = false;
    ...
    local i = _g - 1;
    local variant = variants[i];
    local variantReturns = variant.returns;
    local variantArgs = variant.arguments;
    if (variants.length > 1) then 
      fout:writeString(Std.string(Std.string("") .. Std.string(identation)) .. Std.string("@:overload\n"));
    end;
    fout:writeString(Std.string(Std.string(Std.string(Std.string("") .. Std.string(identation)) .. Std.string("public static function ")) .. Std.string(functionName)) .. Std.string("("));
    if (variantArgs ~= nil) then 
      local _g = 0;
      local _g1 = variantArgs.length;
      while (_g < _g1) do _hx_do_first_2 = false;
        
        _g = _g + 1;
        local i = _g - 1;
        local arg = variantArgs[i];
        local argName = arg.name;
        local argType = Generator.mapType(arg.type);
        fout:writeString(Std.string(Std.string(Std.string("") .. Std.string(argName)) .. Std.string(":")) .. Std.string(argType));
        if (i < (variantArgs.length - 1)) then 
          fout:writeString(", ");
        end;
      end;
    end;
    local returnType = "Void";
    if (variantReturns ~= nil) then 
      if (variantReturns.length == 1) then 
        returnType = Generator.mapType(variantReturns[0].type);
      else
        if (variantReturns.length > 1) then 
          returnType = self:getFunctionStructName(functionName);
        end;
      end;
    end;
    fout:writeString(Std.string(Std.string("):") .. Std.string(returnType)) .. Std.string(";\n\n"));
  end;
end
Generator.prototype.writeEnums = function(self,fout,enums) 
  local _g = 0;
  local _g1 = enums.length;
  while (_g < _g1) do _hx_do_first_1 = false;
    
    _g = _g + 1;
    local i = _g - 1;
    local hxEnum = enums[i];
    fout:writeString(Std.string(Std.string("extern enum ") .. Std.string(Std.string(hxEnum.name))) .. Std.string(" {\n"));
    local _g = 0;
    local _g1 = _hx_wrap_if_string_field(hxEnum.constants,'length');
    while (_g < _g1) do _hx_do_first_2 = false;
      
      _g = _g + 1;
      local i = _g - 1;
      local value = hxEnum.constants[i];
      local valueName = _hx_wrap_if_string_field(value.name,'toUpperCase')(value.name);
      fout:writeString(Std.string(Std.string("\t") .. Std.string(valueName)) .. Std.string(";\n"));
    end;
    fout:writeString("}\n\n");
  end;
end

Generator.prototype.__class__ =  Generator
...
local _hx_static_init = function()
  ...
  Generate.LOVE_API_URL = "https://love2d-community.github.io/love-api/love-api.json";
